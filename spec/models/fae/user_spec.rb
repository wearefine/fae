...
describe Fae::User do
  ...
  describe '.super_admin?' do
    it 'should return true when user is a super admin' do
      super_admin = FactoryGirl.build_stubbed(:fae_role, name: 'super admin')
      super_user = FactoryGirl.build_stubbed(:fae_user, role: super_admin)

      expect(super_user.super_admin?).to eq(true)
    end

    it 'should return false when user is not a super admin' do
      admin = FactoryGirl.build_stubbed(:fae_role, name: 'admin')
      user = FactoryGirl.build_stubbed(:fae_user, role: admin)
  ...
  describe '.user?' do
    it 'should return true when user is a user' do
      user_role = FactoryGirl.build_stubbed(:fae_role, name: 'user')
      user = FactoryGirl.build_stubbed(:fae_user, role: user_role)

      expect(user.user?).to eq(true)
    end

    it 'should return false when user is not a user' do
      admin_role = FactoryGirl.build_stubbed(:fae_role, name: 'admin')
      user = FactoryGirl.build_stubbed(:fae_user, role: admin_role)
  ...
  describe '.super_admin_or_admin?' do
    it 'should return true when user is a super admin' do
      user_role = FactoryGirl.build_stubbed(:fae_role, name: 'super admin')
      user = FactoryGirl.build_stubbed(:fae_user, role: user_role)
      expect(user.super_admin_or_admin?).to eq(true)
    end

    it 'should return true when user is a admin' do
      user_role = FactoryGirl.build_stubbed(:fae_role, name: 'admin')
      user = FactoryGirl.build_stubbed(:fae_user, role: user_role)
    ...
    it 'should return false when user is a user' do
      admin_role = FactoryGirl.build_stubbed(:fae_role, name: 'user')
      user = FactoryGirl.build_stubbed(:fae_user, role: admin_role)
  ...
  describe '.full_name' do
    it 'should return full name' do
      user = FactoryGirl.build_stubbed(:fae_user, first_name: 'John', last_name: 'Doe')
  ...
  describe 'concerns' do
    it 'should allow instance methods through Fae::UserConcern' do
      user = FactoryGirl.build_stubbed(:fae_user)
