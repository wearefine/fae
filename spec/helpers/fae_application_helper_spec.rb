...
describe Fae::ApplicationHelper do
  ...
  describe "#col_name_or_image" do
    ...
    it 'should display original image when filetype is svg' do
      winemaker_svg = FactoryGirl.build(:fae_image_svg, imageable_type: 'Winemaker', imageable_id: 1, attached_as: 'winemaker_image')
      @winemaker = FactoryGirl.create(:winemaker, winemaker_image: winemaker_svg)
  ...
  describe "#change_item_link" do
    ...
    it 'should display a link with data from the fae_dispaly_field if its a string or an integer' do
      ...
      @item = FactoryGirl.build_stubbed(:milestone, year: 2016)
      @change = FactoryGirl.build_stubbed(:fae_change, changeable: @item, user: @user )

      @item2 = FactoryGirl.build_stubbed(:varietal, name: "Example Name")
      @change2 = FactoryGirl.build_stubbed(:fae_change, changeable: @item2, user: @user )
    ...
    it 'should display the changeable_id for the item field if changeable items fae_dispaly_field is nil' do
      ...
      @user = FactoryGirl.build_stubbed(:fae_user)
      @item = FactoryGirl.build_stubbed(:varietal, name: nil)
      @change = FactoryGirl.build_stubbed(:fae_change, changeable: @item, user: @user )
      @item2 = FactoryGirl.build_stubbed(:milestone, year: nil)
      @change2 = FactoryGirl.build_stubbed(:fae_change, changeable: @item2, user: @user )
